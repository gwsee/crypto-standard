...
import (
	...
	"github.com/gwsee/crypto-standard/asym/secp256k1"
	"github.com/gwsee/crypto-standard/hash"
)
...
func (key *ECDSAPublicKey) FromBytes(k []byte, opt int) error {
	...
	if opt == AlgoP256K1Recover || opt == AlgoP256K1 {
		if len(k) == 20 {
			...
			return fmt.Errorf("parse key error, key is %v", hex.EncodeToString(k))
		}
	}
	...
	case AlgoP256R1:
		if len(k) != 65 {
			return fmt.Errorf("r1 key length should be 65")
		}
	...
	case AlgoP521R1:
		...
		key.Y.SetBytes(k[67:])
	default:
		return fmt.Errorf("k1 parse key error: %v, %v", hex.EncodeToString(k), opt)
	}
	return nil
}
